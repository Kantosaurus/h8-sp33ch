...
class MetaClassifier:
    ...
    def set_model_weights(self, X_val, y_val):
        """
        Compute and store F1-score based weights for each base model
        """
        weights = []
        for name, model in self.base_models.items():
            if hasattr(model, 'predict_proba'):
                ...
                preds = (model.decision_function(X_val) > 0).astype(int)
            else:
        ...
        # Normalize so weights sum to 1 (optional)
        weights = np.array(weights)
        self.model_weights = weights / weights.sum()
        print("Base model weights (from F1 scores):", dict(zip(self.base_models.keys(), self.model_weights)))
    ...
    def extract_meta_features(self, X):
        ...
        # Get predictions from all base models
         # Apply F1-score weights if available
        for i, (name, model) in enumerate(self.base_models.items()):
            ...
            if self.model_weights is not None:
                pred_proba = pred_proba * self.model_weights[i]
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None):
        ...
        self.set_model_weights(X_val, y_val)
